...
DATA_DIR = REPO_ROOT / "src" / "analysis"
...
def color_palette(name, count):
    if sns is not None:
        return sns.color_palette(name, count)
    cmap = matplotlib.colormaps.get_cmap({"viridis": "viridis", "Blues_r": "Blues_r", "Set3": "Set3"}.get(name, "tab20"))
...
def fig2_subfield_distribution():
    ...
    colors = color_palette("viridis", len(labels))
...
def fig5_characterization_methods():
    ...
    fig, ax = plt.subplots(figsize=(7, 5))
    colors = color_palette("Blues_r", len(methods))
...
def fig7_journal_distribution():
    ...
    colors = color_palette("Set3", len(labels))
